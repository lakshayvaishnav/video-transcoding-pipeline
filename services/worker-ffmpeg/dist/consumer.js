...
import { KAFKA_TOPICS, } from '@video-transcoder/shared-types';
...
export async function startJobConsumer(kafkaClient, logger) {
    await kafkaClient.createConsumer(config.kafka.groupId, [KAFKA_TOPICS.TRANSCODE_JOBS], async (event, _metadata) => {
        ...
        try {
            ...
            if (profilesToTranscode.length === 0) {
                logger.warn({ jobId, sourceHeight: sourceMetadata.height }, 'All profiles require upscaling. Using smallest profile as fallback.');
            ...
            // Publish completion
            const completedEvent = createEvent('TranscodeCompleted', {
                jobId,
                uploadId,
                userId,
                outputs,
            ...
            const failedEvent = createEvent('TranscodeFailed', {
                jobId,
                uploadId,
                userId,
                error: {
...
async function publishProgress(kafkaClient, jobId, uploadId, userId, progress, message) {
    const progressEvent = createEvent('TranscodeProgress', {
        jobId,
        uploadId,
        userId,
        progress,
