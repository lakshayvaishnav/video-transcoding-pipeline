...
export async function transcodeToProfile(sourcePath, outputDir, profile, logger) {
    ...
    logger.info({ profile: profile.name, resolution: `${profile.width}x${profile.height}` }, 'Starting transcode');
    // Transcode to MP4
    await new Promise((resolve, reject) => {
        ffmpeg(sourcePath)
            ...
            if (progress.percent) {
                logger.debug({ profile: profile.name, percent: progress.percent.toFixed(1) }, 'Transcode progress');
            }
        })
            .on('end', () => resolve())
